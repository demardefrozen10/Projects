...
def get_course_description(course_code):
    ...
    for i in range(pdfdoc_remote.numPages):
        ...
        if description_text.find("course information") != -1:
            ...
            if index >= 5000:
                retval = pdf_text_proper_casing[
                         description_index:description_index + index] + "... see course syllabus for more info: " + url
            else:
...
def get_prerequisites(course_code):
    ...
    for i in range(pdfdoc_remote.numPages):
        ...
        for j in pdf_text[the_index:the_index + 500]:
            ...
            if j == '.' and index > 20:
                if pdf_text[the_index + index + 1:the_index + index + 2] != '0' and pdf_text[
                                                                                    the_index + index + 1:the_index + index + 2] != '5':
                    break
            index += 1
